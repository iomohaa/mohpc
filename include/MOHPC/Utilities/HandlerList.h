...
public: \
template<> MOHPC_EXPORTS fnHandle_t set<c>(c::Type&& handler) { return h.add(std::forward<c::Type>(handler)); }\
template<> MOHPC_EXPORTS void unset<c>(fnHandle_t handle) { h.remove(handle); }
