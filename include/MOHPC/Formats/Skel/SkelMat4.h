...
{
	VecAdd( m1.val[ 0 ], m2.val[ 0 ], val[ 0 ] );
	VecAdd( m1.val[ 1 ], m2.val[ 1 ], val[ 1 ] );
	VecAdd( m1.val[ 2 ], m2.val[ 2 ], val[ 2 ] );
}

inline
void SkelMat4::Difference( const SkelMat4& m1, const SkelMat4& m2 )
{
	VecSubtract( m1.val[ 0 ], m2.val[ 0 ], val[ 0 ] );
	VecSubtract( m1.val[ 1 ], m2.val[ 1 ], val[ 1 ] );
	VecSubtract( m1.val[ 2 ], m2.val[ 2 ], val[ 2 ] );
}
...
void SkelMat4::Inverse( void )
{
	...
	{
		VecMatrixInverse( outmat4x4, mat4x4 );
