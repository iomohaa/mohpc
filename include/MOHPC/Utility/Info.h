...
namespace MOHPC
{
	class MOHPC_UTILITY_EXPORTS InfoIterator
	{
	private:
	...
	class MOHPC_UTILITY_EXPORTS ReadOnlyInfo
	{
	private:
		const char* keyBuffer;
		size_t size;
	...
	class MOHPC_UTILITY_EXPORTS Info
	{
	private:
		char* keyBuffer;
		size_t alloced;
	...
	public:
		...
		void RemoveKey(const char* key, size_t keyLen);
		str ValueForKey(const char* key) const;
		bool BoolValueForKey(const char* key) const;
		uint32_t IntValueForKey(const char* key) const;
		uint64_t LongValueForKey(const char* key) const;
