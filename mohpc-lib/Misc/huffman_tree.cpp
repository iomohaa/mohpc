...
void MOHPC::Huff_putBit( int bit, uint8_t *fout, int *offset) {
	bloc = *offset;
	if ((bloc&7) == 0) {
		fout[(bloc>>3)] = 0;
	}
...
int MOHPC::Huff_getBit( uint8_t *fin, int *offset) {
	int t;
	bloc = *offset;
...
void MOHPC::Huff_addRef(huff_t* huff, uint8_t ch) {
	node_t *tnode, *tnode2;
...
/* Get a symbol */
int MOHPC::Huff_Receive (node_t *node, int *ch, uint8_t *fin) {
...
/* Get a symbol */
void MOHPC::Huff_offsetReceive (node_t *node, int *ch, uint8_t *fin, int *offset) {
	bloc = *offset;
	while (node && node->symbol == INTERNAL_NODE) {
		if (get_bit(fin)) {
			node = node->right;
...
/* Send a symbol */
void MOHPC::Huff_transmit (huff_t *huff, int ch, uint8_t *fout) {
	int i;
	if (huff->loc[ch] == NULL) { 
...
void MOHPC::Huff_offsetTransmit (huff_t *huff, int ch, uint8_t *fout, int *offset) {
	bloc = *offset;
	send(huff->loc[ch], NULL, fout);
	*offset = bloc;
}

void MOHPC::Huff_Decompress(msg_t *mbuf, int offset) {
	int			ch;
	size_t		i, j;
	size_t		cch;
	size_t		size;
...
void MOHPC::Huff_Compress(msg_t *mbuf, int offset) {
	size_t		i;
	int			ch;
	size_t		size;
	uint8_t		seq[65536];
	...
	seq[0] = ( uint8_t )(size>>8);
	seq[1] = ( uint8_t )size&0xff;
...
void MOHPC::Huff_Init(huffman_t *huff) {
