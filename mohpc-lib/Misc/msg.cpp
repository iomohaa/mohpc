...
void MOHPC::MSG_Init( msg_t *buf, uint8_t *data, size_t length ) {
	if (!msgInit) {
		MSG_initHuffman();
	}
...
void MOHPC::MSG_InitOOB( msg_t *buf, uint8_t *data, size_t length ) {
	if (!msgInit) {
		MSG_initHuffman();
	}
	memset (buf, 0, sizeof(*buf));
...
void MOHPC::MSG_Clear( msg_t *buf ) {
	buf->cursize = 0;
...
void MOHPC::MSG_Bitstream( msg_t *buf ) {
	buf->oob = false;
}

void MOHPC::MSG_BeginReading( msg_t *msg ) {
	msg->readcount = 0;
	msg->bit = 0;
	msg->oob = false;
}

void MOHPC::MSG_BeginReadingOOB( msg_t *msg ) {
	msg->readcount = 0;
	msg->bit = 0;
	msg->oob = true;
}

void MOHPC::MSG_Copy(msg_t *buf, uint8_t *data, size_t length, msg_t *src)
{
...
void MOHPC::MSG_WriteBits( msg_t *msg, int value, int bits ) {
	int	i;
//	FILE*	fp;
...
int MOHPC::MSG_ReadBits( msg_t *msg, int bits ) {
	int			value;
	int			get;
	bool	sgn;
	int			i, nbits;
...
void MOHPC::MSG_WriteChar( msg_t *sb, int c ) {
#ifdef PARANOID
	if (c < -128 || c > 127)
...
void MOHPC::MSG_WriteByte( msg_t *sb, int c ) {
#ifdef PARANOID
	if (c < 0 || c > 255)
...
void MOHPC::MSG_WriteData( msg_t *buf, const void *data, size_t length ) {
	size_t i;
	for(i=0;i<length;i++) {
...
void MOHPC::MSG_WriteShort( msg_t *sb, int c ) {
#ifdef PARANOID
...
void MOHPC::MSG_WriteSVC( msg_t *sb, int c ) {
	MSG_WriteBits( sb, c, 8 );
}

void MOHPC::MSG_WriteLong( msg_t *sb, int c ) {
	MSG_WriteBits( sb, c, 32 );
}

void MOHPC::MSG_WriteFloat( msg_t *sb, float f ) {
	union {
		float	f;
		int	l;
	} dat;
...
void MOHPC::MSG_WriteString( msg_t *sb, const char *s ) {
	if ( !s ) {
		MSG_WriteData (sb, "", 1);
	} else {
		size_t	l;
...
void MOHPC::MSG_WriteBigString( msg_t *sb, const char *s ) {
	if ( !s ) {
		MSG_WriteData (sb, "", 1);
	} else {
		size_t	l;
...
void MOHPC::MSG_WriteAngle( msg_t *sb, float f ) {
	MSG_WriteByte (sb, (int)(f*256/360) & 255);
}

void MOHPC::MSG_WriteAngle16( msg_t *sb, float f ) {
...
int MOHPC::MSG_ReadChar (msg_t *msg ) {
	int	c;
...
int MOHPC::MSG_ReadByte( msg_t *msg ) {
	int	c;
...
int MOHPC::MSG_ReadSVC( msg_t *msg ) {
	int	c;
...
int MOHPC::MSG_ReadShort( msg_t *msg ) {
	int	c;
...
int MOHPC::MSG_ReadLong( msg_t *msg ) {
	int	c;
...
void MOHPC::MSG_ReadDir( msg_t *msg, vec3_t dir ) {
	int b;
...
float MOHPC::MSG_ReadCoord( msg_t *msg ) {
	float	sign = 1.0f;
	int		read;
	float	rtn;
...
float MOHPC::MSG_ReadFloat( msg_t *msg ) {
	union {
		uint8_t	b[4];
		float	f;
		int	l;
...
char *MOHPC::MSG_ReadString( msg_t *msg ) {
	static char	string[MAX_STRING_CHARS];
	int		l,c;
	
	l = 0;
	do {
		c = MSG_ReadByte(msg);		// use Readuint8_t so -1 is out of bounds
		if ( c <= 0 ) {
			break;
		}
...
char *MOHPC::MSG_ReadBigString( msg_t *msg ) {
...
char *MOHPC::MSG_ReadStringLine( msg_t *msg ) {
	static char	string[MAX_STRING_CHARS];
	int		l,c;
...
#if 0
void MOHPC::MSG_GetNullEntityState(entityState_t *nullState) {
...
float MOHPC::MSG_ReadAngle8( msg_t *msg ) {
	return BYTE2ANGLE(MSG_ReadShort(msg));
}

float MOHPC::MSG_ReadAngle16( msg_t *msg ) {
	return SHORT2ANGLE(MSG_ReadShort(msg));
}

void MOHPC::MSG_ReadData( msg_t *msg, void *data, int len ) {
	int		i;
...
void MOHPC::MSG_WriteDeltaKey( msg_t *msg, int key, int oldV, int newV, int bits ) {
	if ( oldV == newV ) {
		MSG_WriteBits( msg, 0, 1 );
		return;
	}
...
void MOHPC::MSG_WriteDeltaKeyFloat( msg_t *msg, int key, float oldV, float newV ) {
	if ( oldV == newV ) {
		MSG_WriteBits( msg, 0, 1 );
		return;
	}
...
=====================
*/
void MOHPC::MSG_WriteDeltaUsercmd( msg_t *msg, usercmd_t *from, usercmd_t *to ) {
...
=====================
*/
void MOHPC::MSG_ReadDeltaUsercmd( msg_t *msg, usercmd_t *from, usercmd_t *to ) {
	if ( MSG_ReadBits( msg, 1 ) ) {
...
=====================
*/
void MOHPC::MSG_WriteDeltaUsercmdKey( msg_t *msg, int key, usercmd_t *from, usercmd_t *to ) {
	if ( to->serverTime - from->serverTime < 256 ) {
		MSG_WriteBits( msg, 1, 1 );
		MSG_WriteBits( msg, to->serverTime - from->serverTime, 8 );
	} else {
...
=====================
*/
void MOHPC::MSG_ReadDeltaUsercmdKey( msg_t *msg, int key, usercmd_t *from, usercmd_t *to ) {
	if ( MSG_ReadBits( msg, 1 ) ) {
		to->serverTime = from->serverTime + MSG_ReadBits( msg, 8 );
	} else {
		to->serverTime = MSG_ReadBits( msg, 32 );
...
=================
*/
void MOHPC::MSG_ReportChangeVectors_f( void ) {
	int i;
	for(i=0;i<256;i++) {
		if (pcount[i]) {
			Com_Printf("%d used %d\n", i, pcount[i]);
...
==================
*/
void MOHPC::MSG_WriteDeltaEntity( msg_t *msg, struct entityState_s *from, struct entityState_s *to, 
						   bool force ) {
	int			i, lc;
	int			numFields;
	netField_t	*field;
	...
	lc = 0;
	// build the change vector as uint8_ts so it is endien independent
		...
		switch ( field->type ) {
			...
			case 1: // angles, what a mess! it wouldnt surprise me if something goes wrong here ;)
				...
				if ( bits == 12 ) {
					...
					MSG_WriteBits( msg, ((int)tmp) & ((1<<(uint8_t)bits) -1), bits );
				}
				break;
			case 2: // time
				tmp = *(float *)toF;
...
==================
*/
void MOHPC::MSG_ReadSounds (msg_t *msg, server_sound_t *sounds, int *snapshot_number_of_sounds) {
			...
			*snapshot_number_of_sounds = fubar;
			for (i=0; i<fubar; i++ ) {
				if ( MSG_ReadBits(msg, 1) == 1 ) {
					...
					sounds[i].streamed = (bool)MSG_ReadBits( msg, 1 );
					if ( MSG_ReadBits(msg, 1) == 1 ) {
...
==================
*/
void MOHPC::MSG_WriteSounds (msg_t *msg, server_sound_t *sounds, int snapshot_number_of_sounds) {
...
void MOHPC::MSG_ReadDeltaEyeInfo(msg_t *msg, usereyes_t *from, usereyes_t *to) {
...
void MOHPC::MSG_ReadDeltaEntity( msg_t *msg, entityState_t *from, entityState_t *to, 
						 int number) {
	int			i, lc;
	int			numFields;
	netField_t	*field;
...
//			pcount[i]++;
	...
	VecCopy( to->netorigin, to->origin );
	VecCopy( to->netangles, to->angles );
...
void MOHPC::MSG_WriteDeltaEyeInfo(msg_t  *msg, usereyes_t *from, usereyes_t *to) {
...
=============
*/
void MOHPC::MSG_WriteDeltaPlayerstate( msg_t *msg, struct playerState_s *from, struct playerState_s *to ) {
	int				i;
	playerState_t	dummy;
	int				statsbits;
	int				activeitemsbits;
...
===================
*/
void MOHPC::MSG_ReadDeltaPlayerstate (msg_t *msg, playerState_t *from, playerState_t *to ) {
	int			i, lc;
	int			bits;
	netField_t	*field;
	int			numFields;
