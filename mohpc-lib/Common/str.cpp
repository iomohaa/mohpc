...
MOHPC::str::str(const char* text, size_t len)
	: m_data(nullptr)
{
	assert(text);
	if (*text && len)
...
str MOHPC::operator+(const str& a, const str& b)
{
	str result(a);
...
str MOHPC::operator+(const str& a, const char* b)
{
	str result(a);
...
void MOHPC::str::assign(const char* text, size_t sz)
{
...
void MOHPC::str::resize(size_t len)
{
...
void MOHPC::str::reserve(size_t len)
{
	// just reserve an amount
	EnsureAlloced(len + 1, true);
}

void MOHPC::str::clear()
{
	if (m_data)
	{
		m_data->DelRef();
